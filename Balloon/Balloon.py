...
def main():
    ...
    while time.ticks_diff(time.ticks_ms(), t0) < d0:
        thumby.display.fill(0)
        thumby.display.drawLine(0, H-1, W - int(W * time.ticks_diff(time.ticks_ms(), t0) / d0), H-1, 1)
    ...
    while time.ticks_diff(time.ticks_ms(), t1) < d1:
        ...
        thumby.display.drawLine(W//2-1+int(dx), H-5-size+dy+size, W//2-1+int(dx), H+10+dy, 1)
